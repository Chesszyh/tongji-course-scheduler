...
const store = createStore<State>({
  ...
  mutations: {
    ...
    popStagedCourse(state, payload: string) {
      ...
      state.commonLists.stagedCourses = state.commonLists.stagedCourses.filter(
        (course) => course.courseCode !== payload,
      );
      state.commonLists.selectedCourses = state.commonLists.selectedCourses.filter(
        (course) => course.substring(0, course.length - 2) !== payload,
      );
      state.timeTableData = state.timeTableData.filter(
        (course) => course.code.substring(0, course.code.length - 2) !== payload,
      );
      const codeOfCourse = state.occupied
        .flat()
        .flat()
        .find((item) => item.code.substring(0, item.code.length - 2) === payload)?.code;
    ...
    updateTimeTable(state, payload: courseDetaillet) {
      ...
      if (retOfCanAddCourse.canAdd) {
        const sameCodeCourse = state.timeTableData?.find((course) =>
          isSameCourse(course.code, payload.code),
        );

        // 规定相同课号的课只能有一个
        if (sameCodeCourse) {
          state.timeTableData = state.timeTableData.filter(
            (course) => !isSameCourse(course.code, payload.code),
          );
          ...
          const stagedCourse = state.commonLists.stagedCourses.find(
            (course) => course.courseCode === payload.code.substring(0, payload.code.length - 2),
          );
          if (stagedCourse) {
            // console.log("目标", stagedCourse);
            const targetCourse = stagedCourse.courseDetail.find(
              (course: { code: string; status: number }) =>
                isSameCourse(course.code, payload.code) && course.status === 1,
            );
            if (targetCourse) {
              // console.log("找到了！");
        ...
        // 存到课表数据中
        payload.arrangementInfo.forEach((arrangement) => {
          const courseOnTable = {
            // 每次需要重新创建一个对象，否则会出现引用问题
            showText:
  ...
  getters: {
    isMajorSelected(state) {
      // console.log(',', state.majorSelected);
      return (
        state.majorSelected.calendarId && state.majorSelected.grade && state.majorSelected.major
      );
    },
