...
export function codesToJsonForCSV(codes: string[], rawList: stagedCourse[]): csvCourse[] {
  ...
  for (const code of codes) {
    const course = rawList.find(
      (course) => course.courseCode === code.substring(0, code.length - 2),
    ); // 先找到这门课
    if (course) {
        ...
        for (const arr of targetClass.arrangementInfo) {
          const csvCourse: csvCourse = {
            ...
            teacherName: arr.teacherAndCode
              .split(",")
              .map((str) => str.split("(")[0])
              .join(","), // 复旦的老师只有工号，是 1 系统的问题
            occupyRoom: arr.occupyRoom,
            occucpyWeek: arr.arrangementText.split("]")[0].split("[")[1].split(" ").join("、"),
          };
