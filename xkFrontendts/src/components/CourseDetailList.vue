      ...
      >
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'campus'">
            <div
              :class="getCampusClass(record.campus)"
...
export default {
    ...
    return {
      columns: [
        ...
        },
        {
          ...
          key: "teachers",
          align: "center",
          customRender: ({ text }: { text: teacherlet[] }) =>
            text?.map((teacher) => teacher.teacherName).join(", "),
        },
        {
          title: "校区",
          dataIndex: "campus",
        ...
        },
        {
          ...
          key: "arrangementInfo",
          align: "center",
          customRender: ({ text }: { text: arrangementInfolet[] }) =>
            text?.map((arrangement) => arrangement.arrangementText).join(", "),
        },
        {
          title: "状态",
          dataIndex: "status",
  ...
  computed: {
    localDetailList() {
      ...
      return (
        this.$store.state.commonLists.stagedCourses.find(
          (course) => course.courseCode === this.$store.state.clickedCourseInfo.courseCode,
        )?.courseDetail || []
      );
    },
    title() {
