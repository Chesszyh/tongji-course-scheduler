  ...
  <div class="flex flex-col w-full">
    ...
    <div v-if="selectedType === 'compulsory'">
      <div class="h-150 overflow-auto">
        <a-table
          ...
          :row-selection="{
            selectedRowKeys: localSelectedRowKeys.filter((key: string) =>
              key.startsWith('必_' + courses.grade + '_'),
            ),
    ...
    <div v-else-if="selectedType === 'optional'">
      <a-tabs v-model:activeKey="selectedOptionalType">
        <a-tab-pane
          v-for="type in $store.state.commonLists.optionalTypes"
          :key="type.courseLabelId"
          :tab="type.courseLabelName"
        >
          <div class="h-150 overflow-auto">
            <a-table
              ...
              :data-source="
                filteredCourses(
                  $store.state.commonLists.optionalCourses.find(
                    (item) => item.courseLabelId === type.courseLabelId,
                  )?.courses,
                )
              "
              :pagination="false"
  ...
  methods: {
    ...
    filteredCourses(courses: stagedCourse[]) {
      ...
      courses = courses.filter((course) => {
        return !this.$store.state.commonLists.stagedCourses.some(
          (stagedCourse) => stagedCourse.courseCode === course.courseCode,
        );
      ...
      if (this.searchValue === "") {
        ...
        return courses.filter(
          (course) =>
            course.courseCode.includes(this.searchValue) ||
            course.courseName.includes(this.searchValue),
        );
      }
    },
  },
