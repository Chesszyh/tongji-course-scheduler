<template>
  <div class="teacher-evaluation">
      ...
      >
        ...
        <div class="rating-info mb-2">
          <div class="flex items-center gap-2">
            <a-rate :value="evaluation.rating" :disabled="true" :allowHalf="true" class="text-sm" />
            <span class="text-sm text-gray-600">
              {{ evaluation.rating.toFixed(1) }}
            </span>
          </div>
          <div class="text-xs text-gray-500">基于 {{ evaluation.reviewCount }} 条评价</div>
        </div>
    ...
    <div v-else-if="selectedCourse && selectedCourse.courseCode" class="no-evaluation">
      <a-empty :image="simpleImage" description="暂无教师评价数据" class="py-6">
        <template #description>
          <span class="text-gray-500 text-sm"> 该课程的教师评价数据暂未收录 </span>
        </template>
      </a-empty>
    </div>
    ...
    <div v-else class="no-selection">
      <a-empty :image="simpleImage" description="请选择课程查看教师评价" class="py-6">
        <template #description>
          <span class="text-gray-500 text-sm"> 点击左侧课程可查看相关教师评价 </span>
        </template>
      </a-empty>
    </div>
    ...
    <!-- 数据来源说明 -->
    <div
      class="data-source mt-4 p-2 bg-yellow-50 border border-yellow-200 rounded text-xs text-yellow-700"
    >
...
export default {
  ...
  methods: {
    ...
    async mockFetchEvaluations(_courseCode: string) {
      ...
      this.teacherEvaluations = _courseCode ? mockEvaluations : [];
    },
  },
};
</script>
